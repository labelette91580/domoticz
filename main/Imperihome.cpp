...
namespace http {
	namespace server {
		void CWebServerHelper::GetScenes(Json::Value& root)//called from imperihome
		{
			request request_;
			WebEmSession session;

			if (plainServer_) { // assert
				plainServer_->Cmd_GetScenes(session, request_, root);
			}
#ifdef NS_ENABLE_SSL
			else if (secureServer_) {
				secureServer_->Cmd_GetScenes(session, request_, root);
			}
#endif
		}
