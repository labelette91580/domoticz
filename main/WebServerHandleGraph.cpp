				...
				if (sensor == "temp")
				{
					...
					if (!result.empty())
					{
						...
						for (const auto& sd : result)
						{
							...
							if (((dType == pTypeWIND) && (dSubType == sTypeWIND4)) || ((dType == pTypeWIND) && (dSubType == sTypeWINDNoTemp)) || (dType == pTypeSetpoint && dSubType == sTypeSetpoint && sd[1] != "0.0"))
							{
								double tvalue = ConvertTemperature(atof(sd[1].c_str()), tempsign);
								root["result"][ii]["ch"] = tvalue;
							}
							if ((dType == pTypeHUM) || (dType == pTypeTEMP_HUM) || (dType == pTypeTEMP_HUM_BARO) || (dType == pTypeSetpoint && dSubType == sTypeSetpoint && sd[1] != "0.0") || ((dType == pTypeThermostat6) && ((dSubType == sTypeThermostat6TempHum) || (dSubType == sTypeThermostat6TempHumBaro))))
							{
