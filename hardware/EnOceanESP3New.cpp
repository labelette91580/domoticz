...
void CEnOceanESP3::TestData(ESP3_RORG_TYPE rorg, unsigned int sID,  unsigned char status , enocean::T_DATAFIELD * OffsetDes, ...)
{
	...
	*buff++ = status ; //status
	...
	ParseESP3Packet(PACKET_RADIO_ERP1, mBuffer, mdatalen, &mBuffer[mdatalen], 0);

}
void CEnOceanESP3::TestData(char * sdata, uint8_t pType,  const char * optData)
{
    char lBuffer[2014];
    uint8_t mBuffer[2014];
    ...
    if (sdata[0] == 'P')
    {
        pType = (ESP3_PACKET_TYPE)(sdata[1] - '0');
        sdata +=2 ;
    }
	...
	ParseESP3Packet(pType, mBuffer, mdatalen, &mBuffer[mdatalen], moptionallen);
}
...
void CEnOceanESP3::dumpPacket(uint8_t *data, uint16_t datalen)
{
...
//	return sstr.str();
    Debug(DEBUG_HARDWARE, "Read: %s", sstr.str().c_str());
}
...
void CEnOceanESP3::updateCrc(uint8_t * buffer, int len )
{
		...
		//if (crc==0)
        {
			...
			std::string sdata = FormatESP3Packet( packettype, data,  datalen, optdata,  optdatalen);
